...
do
    ...
    function onLBSimulatorTick(simulator, ticks)
        ...
        simulator:setInputBool(19, true)
        simulator:setInputNumber(20, 6)
...
require("Helpers.icon_engine_display")
...
-- User Inputs
-- 17: Fuel Total
-- 19: Hybrid Vehicle: boolean
-- 20: Number of Gears: number
...
function onDraw()
    ...
    if isHybrid then
        ...
        sColor(COLOR_blue)
        screen.drawText(8, topMargin - 3, "ENG")
        -- Electric Engine Throttle Line
        sColor(COLOR_darkBlue)
        screen.drawRectF(1, 7, 29, 3)
        sColor(COLOR_blue)
        local electricEngineThrottleLine = round((electricEngineThrottle - 0) * (31 - 4) / 100)
        DRF(2, 8, electricEngineThrottleLine, 1)
        DRF(2, 7, 1, 1)
        DRF(8, 7, 1, 1)
        DRF(15, 7, 1, 1)
        sColor(COLOR_lightBlue)
        DRF(22, 7, 1, 1)
        DRF(28, 7, 1, 1)

        display_icon_engine(topMargin + 7, 0, engRPS)
    else
        display_icon_engine(topMargin, 0, engRPS)
    end
